...
def evaluateSentence(lCandidate, lReference):
   ...
   while lCandidate and lReference:
      if lCandidate[0][0] == lReference[0][0]:  # words right
         # print(lCandidate[0][0], indexCandidate, indexReference)
         nCorrectWords += 1
...
def readNonEmptySentenceList(sents, bIgnoreNoneTag=True, is_cws:bool=False):
    if is_cws:
      return [[(ii, "-NONE-") for ii in sent.split(' ')] for sent in sents]
   #  else:
   #    return [[(ii.rsplit('/', 1)[0],  "-NONE-" if len(ii.split('/')) == 1 else ii.rsplit('/', 1)[1]) for ii in sent.split(' ')] for sent in sents] 
    out = []
    for sent in sents:
        lNewLine = []
        lLine = sent.split(' ')
...
def score(sReference, sCandidate, tag_num=1, verbose=False, is_cws:bool=False):
   ...
   #  echo(4, nTotalCorrectTags, nCandidateWords, nReferenceWords)
...
if __name__ == '__main__':
    ...
    refs = [ii.strip() for ii in open('pos_gold.txt',encoding='utf-8').readlines()]
    output=[]
    cads = [ii.strip() for ii in open('pos_test.txt',encoding='utf-8').readlines()]
